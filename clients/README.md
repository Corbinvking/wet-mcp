...
Every example is retained for a possible future hosted Streamable HTTP release. Production currently
returns HTTP `503`/`mcp_release_held` before discovery or calls. Do not install, copy, configure,
connect to, or call these examples while the hold is active. Owner authorization, legal review, and
source-rights clearance must all precede a final release action. After a future authorized release,
merge the relevant object into the client file named below; do not overwrite unrelated server
entries.

| Future post-clearance example | Typical location or flow |
|---|---|
...
| `gemini-cli.json` | Project or user Gemini `settings.json`; the future authorized extension would use `gemini-extension.json` at repository root |
...
Client formats change independently. The W.E.T.-owned
[worldeventtrading.com/mcp](https://www.worldeventtrading.com/mcp) page is the current release-hold
record and will become the supported installation guide only after final authorization.
