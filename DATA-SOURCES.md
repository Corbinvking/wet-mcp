...
Production currently returns route-wide HTTP `503`/`mcp_release_held` before discovery or calls. In a controlled candidate environment, policy `mcp-source-rights/2026-09-05.phase1` is default-deny: six W.E.T.-sourced/derived tools return typed `source_rights_pending`; only `wet_resolve` is the caller-supplied-text exception. Credentials, paid access, adapters, and environment settings cannot bypass either hold.
