...
The current default-deny source-rights refusal is `source_rights_pending` under policy `mcp-source-rights/2026-09-05.phase1`. It applies to six W.E.T.-sourced/derived tools and returns policy and exclusion metadata with no market or index value fields. It is protocol-safe but not a useful sourced result or launch-readiness pass. Credentials, paid access, adapters, and environment settings cannot bypass it.
